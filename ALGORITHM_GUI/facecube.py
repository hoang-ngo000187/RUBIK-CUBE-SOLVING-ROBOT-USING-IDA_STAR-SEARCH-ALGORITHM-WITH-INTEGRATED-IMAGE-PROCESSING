...
# Mapping vị trí góc đến vị trí các viên trên các mặt
corner_facelet = (
    (Facelet.U9, Facelet.R1, Facelet.F3), # URF = (8, 9, 20)
    (Facelet.U7, Facelet.F1, Facelet.L3), # UFL = (6, 18, 38)
    (Facelet.U1, Facelet.L1, Facelet.B3), # UFB = (, , )
...
# Mapping vị trí cạnh đến vị trí các viên trên các mặt
edge_facelet = (
...
class FaceCube:
    ...
    def to_string(self):
        """Chuyển từ facecube sang cube_string"""
        return "".join(Color(i).name for i in self.f)

    def to_cubiecube(self):
        """Chuyển từ facecube sang cubiecube"""
        cc = cubiecube.CubieCube()
        for i in range(8):
            # Tất cả góc đều bắt đầu bằng U hoặc D nên ta có thể tìm định hướng của bất kỳ góc nào đã cho
            for ori in range(3):
